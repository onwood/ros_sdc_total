...
//////////////////추가한 부분//////////////////
#include "object_detection_gl_ros/CoordArray.h"
#include "object_detection_gl_ros/Coord.h"
#include "object_detection_gl_ros/ObjectWidthArray.h"
#include "object_detection_gl_ros/ObjectWidth.h"
#include "object_detection_gl_ros/ObjectCount.h"
...
private:
    ...
    //////////////////추가한 부분//////////////////
    ros::Publisher coord_data_pub;
    ros::Publisher object_count_pub;
    ros::Publisher object_width_pub;
    std::string coord_data_topic = std::string("coord_data");
    //std::string object_count_topic = std::string("object_count");
    std::string object_width_topic = std::string("object_width");
...
void ObjectDetectionGlRosNode::InitROS(void)
{
    ...
    //////////////////추가한 부분//////////////////
    coord_data_pub = nh.advertise<object_detection_gl_ros::CoordArray>(coord_data_topic, 1);
    //object_count_pub = nh.advertise<object_detection_gl_ros::ObjectCount>(object_count_topic, 1);
    object_width_pub = nh.advertise<object_detection_gl_ros::ObjectWidthArray>(object_width_topic, 1);
...
void ObjectDetectionGlRosNode::PubMarker(void) 
{
    ...
    //////////////////추가한 부분//////////////////
    object_detection_gl_ros::CoordArray coord_data_msg;
    object_detection_gl_ros::Coord coord_msg;
    object_detection_gl_ros::ObjectWidthArray object_width_msg;
    object_detection_gl_ros::ObjectWidth width_msg;
    //object_detection_gl_ros::ObjectCount object_count_msg;
    ...
    //////////////////추가한 부분//////////////////
    //object_count_msg.size = obj_detect.obj_info_vector.size();

    //object_count_pub.publish(object_count_msg);
...
/*
	ROS_INFO("marker_msg.pose.position.x = %f", obj_detect.obj_info_vector[i].cur_point[0]);
	ROS_INFO("marker_msg.pose.position.y = %f", obj_detect.obj_info_vector[i].cur_point[1]);

	ROS_INFO("coord_msg.x = %f", obj_detect.obj_info_vector[i].cur_point[0]);
	ROS_INFO("coord_msg.y = %f", obj_detect.obj_info_vector[i].cur_point[1]);
*/
	//////////////////추가한 부분//////////////////
	coord_msg.x = obj_detect.obj_info_vector[i].cur_point[0];
	coord_msg.y = obj_detect.obj_info_vector[i].cur_point[1];
...
/*
	...
	for(int i=0;i<5;i++) {
		coord_msg.x = x[i];
		coord_msg.y = y[i];
		coord_data_msg.coords.push_back(coord_msg);
	}
*/
	//coord_msg.x = marker_msg.pose.position.x;
	//coord_msg.y = marker_msg.pose.position.y; // 값 한번 썼다고 안 뺏김
	...
	//////////////////추가한 부분//////////////////
	width_msg.width = obj_detect.obj_info_vector[i].rightbottom_point[0] -obj_detect.obj_info_vector[i].lefttop_point[0]; // x2 - x1 = 너비

	object_width_msg.widths.push_back(width_msg);

	//object_width_pub.publish(object_width_msg);
    ...
    //////////////////추가한 부분//////////////////
    coord_data_pub.publish(coord_data_msg);
    //object_count_pub.publish(object_count_msg);
    object_width_pub.publish(object_width_msg);
